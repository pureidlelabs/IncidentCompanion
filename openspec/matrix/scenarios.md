...
| A change reaches every open screen, and says only what changed | Another analyst writes from outside the case | demonstrated | server/test/an-announcement-names-its-writer.test.ts :: an announcement of another analyst's write > names an analyst writing from outside the room by their name |
| A change reaches every open screen, and says only what changed | A writer whose account is gone | demonstrated | server/test/an-announcement-names-its-writer.test.ts :: an announcement of another analyst's write > names nobody for a writer whose account is gone before the save |
...
| Written prose is edited together, not saved over | One of the writers loses write before the words are stored | demonstrated | server/test/words-typed-together-outlive-one-writer-losing-reach.test.ts :: two analysts writing one note, one losing write > stores both sets of words through the analyst who still writes |
| Written prose is edited together, not saved over | Every writer's account is gone before the words are stored | demonstrated | server/test/words-whose-every-writer-is-gone-are-stored.test.ts :: words whose writers are gone before they are stored > stores them on the quiet moment, naming nobody ; server/test/words-whose-every-writer-is-gone-are-stored.test.ts :: words whose writers are gone before they are stored > stores them when the last reader leaves, naming nobody ; server/test/words-whose-every-writer-is-gone-are-stored.test.ts :: words whose writers are gone before they are stored > stores them as the application shuts down, naming nobody |
| Written prose is edited together, not saved over | The only writer loses write before the words are stored | demonstrated | server/test/words-whose-every-writer-is-gone-are-stored.test.ts :: words whose writers are gone before they are stored > does not store words whose only writer lost write and still has an account |
