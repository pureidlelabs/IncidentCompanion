...
**That second page's text is not a diagnosis, whatever it says.** Its handler prints the same advice about `allowedHosts` and the hostname for every failure to load the preview script, under two conditions and no others: the host is not the literal `localhost`, and the config is a development one. `127.0.0.1` is never `localhost`, so this tier always meets the first — but **at `localhost` the handler draws nothing at all**, which is worse than the red message, and against a built Storybook the branch is dead. So `brokenPreview` asks the server again and reports what it answers: a status when it refused, a throw when the connection did, and `re-fetched 200` when the entry script itself is fine. **That last one is not proof the failure was transient** — the `error` event fires for a failure anywhere in the module graph, so a story file or decorator that will not resolve leaves the entry script perfectly serveable.
