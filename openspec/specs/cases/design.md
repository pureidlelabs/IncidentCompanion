...
**A case can therefore be moved out of the mover's own reach.** That is a boundary rather than an oversight: they already reached the case, so the move gains them nothing, and the act is recorded against both customers. The store performs the move, because the row the move leaves behind is one the mover may no longer see, and the store's move holds the move's rules itself, deciding them from who is asking before it changes anything: the mover writes the case where it is now, the destination is not the default customer, and a case carrying a reference lands only with a customer the mover reaches, refused the same way whatever that customer holds. Undoing it is not free — being an administrator carries no reach over a case, so whoever moves it back reaches the destination through a group like anybody else.
