...
- **An element that takes no pointer is not a target, unless it is merely disabled.** `overlap`, `offscreen` and `small-target` skip anything whose computed `pointer-events` is `none` — a marker like `OverlayAnchor`, painted over the thing it points at so the collision is the design. **Disabled controls are the exception and it is load-bearing**: this kit draws `isDisabled` with `pointer-events-none`, so excluding on the property alone would drop every dimmed button, tab and link from all three checks, and a disabled control is painted at full size and can be undersized or cut off exactly as an enabled one can. The predicate is `[disabled], .disabled, [aria-disabled="true"]`, the same one the contrast check uses. What no check can see is a control made unclickable by mistake while claiming to be enabled.
