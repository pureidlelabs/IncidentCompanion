...
describe.skipIf(!db)('a version-checked write', () => {
  ...
  it('advances the version by exactly one per accepted write', async () => {
    // **The SQL increment is defensive, not the mechanism.** Replacing it
    // with `expectedVersion + 1` leaves this green, because the `where`
    // guarantees those are the same number -- so a name claiming two writers
    // cannot compute the same next version would be a lie.
    //
