...
| A case's destruction is itself a record | An analyst deletes a case | undemonstrated | |
...
| Requirement | Scenario | Status | Evidence or reason |
| --- | --- | --- | --- |
| It comes up with one command and no preparation | A first start on a clean machine | undemonstrated | |
...
| The connection is protected, and there is no way to turn that off | An install has no certificate | undemonstrated | |
...
| Nothing is written until an analyst has approved it | An import is previewed | undemonstrated | |
...
| An imported row says that it was imported, and that nobody has read it | An imported row is read back | undemonstrated | |
...
| An install can be given its library as a document, and can read it back | An operator exports a library | undemonstrated | |
...
| Requirement | Scenario | Status | Evidence or reason |
| --- | --- | --- | --- |
| A connection is admitted by its own checks, and their absence is silent | A connection is opened from another site | undemonstrated | |
...
| A claim warns; it does not lock | Somebody writes to a claimed entry | undemonstrated | |
...
| Written prose is edited together, not saved over | An analyst writes while disconnected | undemonstrated | |
...
| A refusal says which of the caller's problems it is | A caller asks for something out of reach | undemonstrated | |
| A refusal says which of the caller's problems it is | A caller sends a body the interface cannot accept | undemonstrated | |
