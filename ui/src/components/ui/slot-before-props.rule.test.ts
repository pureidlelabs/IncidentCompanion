...
/**
 * The rule that keeps a component's `data-part` from eating the caller's,
 ...
 * the call site wrote. It is found by a story assertion going red, or not at
 * all.
 *
 * The invalid cases are shapes this kit writes rather than inventions: a plain
 * `{...props} data-part`, a conditional spread between the two, and the
 * commented form that gets it right.
 */
...
tester.run('local/slot-before-props refuses a data-part a spread can overwrite', rule, {
  valid: [
    // The shape the whole kit is being moved onto.
    { code: '<AriaLink data-part="link" {...props} />' },
    // No spread at all: nothing can overwrite it.
    { code: '<span data-part="button-pending" aria-hidden />' },
    { code: '<AriaButton data-part="button" className={c}>{children}</AriaButton>' },
    ...
    { code: '<AriaButton data-part="button" {...(refused ? { disabled: true } : {})} />' },
    // A `data-part` read out of a variable name is not this attribute.
    ...
    {
      code: `<span data-part="avatar" role="img" {...props} className={c}>
        <img src={src} alt="" data-part="avatar-image" className={i} />
        <span aria-hidden data-part="avatar-fallback" className="leading-none">{f}</span>
      </span>`,
    },
  ],
  ...
  invalid: [
    {
      code: '<AriaLink {...props} data-part="link" />',
      errors: [{ messageId: 'after' }],
      output: '<AriaLink data-part="link" {...props} />',
    },
    // Two spreads with the attribute between them: still overwritten by the
    // first, and the second is not what put it there.
    {
      code: '<AriaButton {...props} data-part="button" {...extra} />',
      errors: [{ messageId: 'after' }],
      output: '<AriaButton data-part="button" {...props} {...extra} />',
    },
