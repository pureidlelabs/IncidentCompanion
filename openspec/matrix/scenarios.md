...
| An install always has somebody who can administer it | Two administrators act on each other at once | demonstrated | server/test/two-administrators-cannot-strand-the-install.test.ts :: two administrators acting on each other at once > lets one act and refuses the other, leaving an administrator in every round |
...
| Authentication resists guessing, and says so to the auditor | A familiar address falls out of use | demonstrated | server/test/a-familiar-address-has-a-stated-life.test.ts :: how long an address stays familiar > counts an address unused for more than 90 days in the unfamiliar run |
| Authentication resists guessing, and says so to the auditor | Signing in keeps an address familiar | demonstrated | server/test/a-familiar-address-has-a-stated-life.test.ts :: how long an address stays familiar > starts an address's 90 days again at each right password from it |
| Authentication resists guessing, and says so to the auditor | An account signs in from more addresses than it keeps | demonstrated | server/test/a-familiar-address-has-a-stated-life.test.ts :: how long an address stays familiar > forgets the least recent of 20 familiar addresses when a 21st gives the right password |
| Authentication resists guessing, and says so to the auditor | Addresses no longer familiar are removed | demonstrated | server/test/a-familiar-address-has-a-stated-life.test.ts :: how long an address stays familiar > prunes what is no longer familiar, and records the pass by count ; server/test/a-familiar-address-has-a-stated-life.test.ts :: how long an address stays familiar > keeps every address when the pruning cannot be recorded ; server/test/a-familiar-address-has-a-stated-life.test.ts :: how long an address stays familiar > prunes at boot, and keeps a daily schedule |
...
| Administrative events are logged | A removal that removes nothing | demonstrated | server/test/an-act-that-changed-nothing-is-not-logged-as-done.test.ts :: an administrative act that changed nothing > refuses a revocation of nobody in the group as not there, and logs no revocation ; server/test/an-act-that-changed-nothing-is-not-logged-as-done.test.ts :: an administrative act that changed nothing > refuses a release of a customer the group does not hold as not there, and logs no release |
| Administrative events are logged | A change to what already stands | demonstrated | server/test/an-act-that-changed-nothing-is-not-logged-as-done.test.ts :: an administrative act that changed nothing > logs a hold and a grant once however often and however concurrently asked, and a revocation and a release that happened ; server/test/an-act-that-changed-nothing-is-not-logged-as-done.test.ts :: an administrative act that changed nothing > logs no account state or role the account already had, however concurrently asked ; server/test/an-act-that-changed-nothing-is-not-logged-as-done.test.ts :: an administrative act that changed nothing > leaves the last line agreeing with the state, however disables and enables interleave |
| Administrative events are logged | An act half done is asked for again | demonstrated | server/test/an-act-that-changed-nothing-is-not-logged-as-done.test.ts :: an administrative act that changed nothing > finishes an act whose library half failed when it is asked again, and records it once |
...
| Words the install cannot yet store are said to be unsaved | A save of written words fails | demonstrated | server/test/unsaved-words-are-told-to-their-writers.test.ts :: words the install holds unsaved > tells every analyst holding the note that its words are unsaved, lets them go on writing, and tells them once they are saved ; ui/src/app/case/a-note-held-unsaved-is-told-to-its-writer.test.tsx :: a note whose words the install holds unsaved > tells its writer, offers the text to copy, and lets them go on writing |
| Words the install cannot yet store are said to be unsaved | The words are stored after all | demonstrated | server/test/unsaved-words-are-told-to-their-writers.test.ts :: words the install holds unsaved > tells every analyst holding the note that its words are unsaved, lets them go on writing, and tells them once they are saved ; ui/src/app/case/a-note-held-unsaved-is-told-to-its-writer.test.tsx :: a note whose words the install holds unsaved > takes the notice away once the install says the words are saved |
| Words the install cannot yet store are said to be unsaved | A passage is opened while its words are unsaved | demonstrated | server/test/unsaved-words-are-told-to-their-writers.test.ts :: words the install holds unsaved > tells a screen opening the note while its words are unsaved |
| Words the install cannot yet store are said to be unsaved | The words can no longer be stored | demonstrated | server/test/unsaved-words-are-told-to-their-writers.test.ts :: words the install holds unsaved > tells the analysts holding the note once its words can no longer be stored ; ui/src/app/case/a-note-held-unsaved-is-told-to-its-writer.test.tsx :: a note whose words the install holds unsaved > says it once the words are given up, and offers the copy again |
| Words the install cannot yet store are said to be unsaved | An analyst leaves with the words unsaved | demonstrated | ui/src/app/case/a-note-held-unsaved-is-told-to-its-writer.test.tsx :: a note whose words the install holds unsaved > asks before its writer leaves with the words unsaved, and leaves once told to |
| A case is one case however it is named | An analyst names the case in another spelling | demonstrated | server/test/a-case-is-one-room-however-it-is-spelled.test.ts :: a case named in capitals over the live connection > puts the analyst in the roster the others see ; server/test/a-case-is-one-room-however-it-is-spelled.test.ts :: a case named in capitals over the live connection > refuses the case deletion while they are connected, by either spelling of the route ; server/test/a-case-is-one-room-however-it-is-spelled.test.ts :: a case named in capitals over the live connection > holds one claim on an entry whichever spelling each analyst claims or releases it by |
| A case is one case however it is named | Words written through another spelling | demonstrated | server/test/a-case-is-one-room-however-it-is-spelled.test.ts :: a case named in capitals over the live connection > edits one document whichever spelling each writer used ; server/test/a-case-is-one-room-however-it-is-spelled.test.ts :: a case named in capitals over the live connection > stores the words the connection accepted, named for their writer |
| A case is one case however it is named | A report is sent by another spelling of its identifier | demonstrated | server/test/a-case-is-one-room-however-it-is-spelled.test.ts :: a case named in capitals over the live connection > sends the words typed just before a send that names the report in capitals |
| A caret names the analyst the install admitted | A caret carries another analyst's name | demonstrated | server/test/a-caret-carries-the-name-the-install-knows.test.ts :: a caret drawn from the live connection > shows the others the name of the analyst who sent it, whatever name it carried |
| A caret names the analyst the install admitted | A connection sends for a caret another analyst holds | demonstrated | server/test/a-caret-carries-the-name-the-install-knows.test.ts :: a caret drawn from the live connection > leaves a caret another connection announced where it was, and relays nothing sent for it ; server/test/a-caret-carries-the-name-the-install-knows.test.ts :: a caret drawn from the live connection > keeps a caret for its analyst across a reconnect, whoever announces it in between ; server/test/a-caret-carries-the-name-the-install-knows.test.ts :: a caret drawn from the live connection > holds nothing for an update it could not read |
| A connection is held to a rate | One analyst opens many connections at once | demonstrated | server/test/a-connection-has-a-bounded-rate.test.ts :: a connection to the case, at a rate > admits a bounded number of one analyst's upgrades made at once, and records each refusal once ; server/test/a-connection-has-a-bounded-rate.test.ts :: a connection to the case, at a rate > counts nothing against an analyst for upgrades they abandoned before the answer |
| A connection is held to a rate | Many connections nobody signed | demonstrated | server/test/a-connection-has-a-bounded-rate.test.ts :: a connection to the case, at a rate > refuses anonymous upgrades past a bound per address, and writes a bounded number of lines ; server/test/a-case-is-one-room-however-it-is-spelled.test.ts :: a case named in capitals over the live connection > records a refused connection and a refused request against the case in one spelling |
| A connection is held to a rate | A connection sends faster than a screen does | demonstrated | server/test/a-connection-has-a-bounded-rate.test.ts :: a connection to the case, at a rate > ends a connection that sends past its budget, before much of it reaches anybody |
| A connection is held to a rate | A screen used hard | demonstrated | server/test/a-connection-has-a-bounded-rate.test.ts :: a connection to the case, at a rate > never limits the application's own client, used hard |
...
| What the install generates in a document leaving it carries no live indicator | A report leaves the install | demonstrated | server/test/case-rows-reach-every-output.test.ts :: what a case holds reaches what it publishes > prints every indicator the case holds in the rendered report ; server/test/case-rows-reach-every-output.test.ts :: what a case holds reaches what it publishes > prints a web address written in a record as no live link ; server/test/a-report-carries-no-live-address-in-any-form.test.ts :: a report carries no live address in any form > the markdown export > writes every spelling of an address defanged and none live ; server/test/a-report-carries-no-live-address-in-any-form.test.ts :: a report carries no live address in any form > the word export > writes every spelling of an address defanged and none live ; server/test/a-report-carries-no-live-address-in-any-form.test.ts :: a report carries no live address in any form > the pdf export > writes every spelling of an address defanged and none live ; server/test/a-report-carries-no-live-address-in-any-form.test.ts :: a report carries no live address in any form > leaves nothing in the markdown export that GitHub-flavoured Markdown autolinks |
| What the install generates in a document leaving it carries no live indicator | A preserved document is read in with a live address | demonstrated | server/test/an-archived-report-is-sent-and-preserved-or-neither.test.ts :: a report an archive says was sent > contains a live indicator an archive preserved, on the way in ; server/test/an-archived-report-is-sent-and-preserved-or-neither.test.ts :: a report an archive says was sent > contains a link on a generated run, on the way in ; server/test/an-archived-report-is-sent-and-preserved-or-neither.test.ts :: a report an archive says was sent > contains a section it says the analyst wrote, on the way in ; server/test/an-archived-report-is-sent-and-preserved-or-neither.test.ts :: a report an archive says was sent > contains a code block it says is verbatim, on the way in ; server/test/an-archived-report-is-sent-and-preserved-or-neither.test.ts :: a report an archive says was sent > stores a genuine preserved report as it arrived, and exports it with no live address |
...
| A refusal says which of the caller's problems it is | A caller removes something that is not there | demonstrated | server/test/a-delete-of-no-row-is-not-a-conflict.test.ts :: a single-row delete > answers not there for an id no row has and for a row of another case, alike |
| A refusal says which of the caller's problems it is | A caller removes something another has changed | demonstrated | server/test/a-delete-of-no-row-is-not-a-conflict.test.ts :: a single-row delete > answers somebody wrote first for a row changed since it was read, naming the version it holds |
