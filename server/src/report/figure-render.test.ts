 ...
 *
 * **This is where the placement itself is asserted.** A figure's placed size
 * is settled here - before the tree is frozen and before either painter sees
 * it - and the whole design rests on that: the page ruler paginates from the
 * same node the PDF does, so the preview's page index and the delivered file
 * agree. The document tier's tests take a placed node as given, so the step
 * that places it is covered nowhere else.
 *
...
describe.skipIf(!db)('placing a figure', () => {
  ...
  /**
   * **The route a client writes a figure's evidence through.** A body naming
   * `evidenceId` lands via the generic collection service; the schema test
   * proves a column of that name exists, and proves nothing about a write
   * reaching it.
   */
   ...
   * an unnormalised webp takes out the whole report's PDF and page ruler.
   */
  ...
  /**
   * **A frozen report gets normalised bytes too.** A re-encode reached only
   * below the frozen return leaves a report sent while holding a webp serving
   * raw bytes for ever - its `.pdf` and page ruler throwing for the life of the
   * report, with no unlock route to fix it. The webp is the whole point of this
   * case: with a png it passes either way.
   */
   ...
   * The header reports a size perfectly happily, so a re-encode outside that
   * `try` hands the painter a size and no drawable bytes. A `.pdf` is the
   * easier half - it throws at `metadata()`.
   */
  it.each([
