 ...
 *
 * **One boundary, both directions, and it is this file.** `ComplianceControl`
 * renders and emits a joined string for every kind - that is what `join` on the
 * descriptor is *for*, and what lets one control serve a member-state picker
 * and a yes/no ground. The record stores a `jsonb` array for the sets, a
 * nullable integer for the counts and `null` for a question nobody has
 * answered. Doing the conversion at each call site is how one kind ends up
 * sending `"AT,BE"` to a column expecting an array, which is a 400 the screen
 * has no way to explain.
 *
 * **`''` becomes `null`, never the empty string.** The absence of an answer is
