...
| Words the install cannot yet store are said to be unsaved | A save of written words fails | demonstrated | server/test/unsaved-words-are-told-to-their-writers.test.ts :: words the install holds unsaved > tells every analyst holding the note that its words are unsaved, lets them go on writing, and tells them once they are saved ; ui/src/screens/notes-unsaved.test.tsx :: a note whose words the install holds unsaved > says so beside the editor, offers the text to copy, and leaves the editor writable |
| Words the install cannot yet store are said to be unsaved | The words are stored after all | demonstrated | server/test/unsaved-words-are-told-to-their-writers.test.ts :: words the install holds unsaved > tells every analyst holding the note that its words are unsaved, lets them go on writing, and tells them once they are saved ; ui/src/screens/notes-unsaved.test.tsx :: a note whose words the install holds unsaved > takes the notice away once the install says the words are saved |
...
| Words the install cannot yet store are said to be unsaved | The words can no longer be stored | demonstrated | server/test/unsaved-words-are-told-to-their-writers.test.ts :: words the install holds unsaved > tells the analysts holding the note once its words can no longer be stored ; ui/src/screens/notes-unsaved.test.tsx :: a note whose words the install holds unsaved > says it once the words are given up, and offers the copy again |
| Words the install cannot yet store are said to be unsaved | An analyst leaves with the words unsaved | demonstrated | ui/src/screens/notes-unsaved.test.tsx :: a note whose words the install holds unsaved > asks before the analyst leaves with the words unsaved, and leaves once told to |
