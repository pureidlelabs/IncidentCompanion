...
describe('a stored indicator reaches the export', () => {
  /**
   * **An empty STIX bundle is well-formed**, so a `collect` reading columns the
   * table does not have produces nothing and every bundle assertion in this file
   * stays green. This is the case that says the rows arrive at all.
   */
...
describe('the STIX bundle', () => {
  ...
  it('refuses a TLP nobody defined', () => {
    ...
    // **A key on `Object.prototype` is not a TLP.** A plain-object lookup answers
    // `constructor` with a function, which is truthy, so the markings have to be
    // held in something that returns only what was put in it. The controller
    // refuses an unknown marking first; this contract has to hold on its own.
