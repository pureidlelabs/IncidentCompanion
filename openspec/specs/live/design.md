...
**An acceptance lasts an hour after it was written or last kept current, and authorises nothing after.** The store dates an acceptance itself, as the moment it is written, and nobody may date one otherwise. Only its own writer, or the prose role for the record it accepts, may remove a current acceptance. The application sees no acceptance of a case its caller does not reach, current or lapsed.

**A save that fails keeps its acceptances, and its writers, and keeps them current.** Every failed save brings the acceptances its document holds up to the present, as the prose role, and a document held with words unsaved tries again every five minutes. So a held acceptance never lapses, however long the store refuses the save, and whatever its writer can reach by then: reach was decided when the word was accepted, and keeping the word current is not a second decision. The same holds whether the writer lost write or had their account disabled, since neither is asked again. An acceptance lapses only when nothing holds its words: the application stopped, or the document was dropped with a save still failing. Words whose acceptance has lapsed cannot be stored; the failure is logged saying so, and the words go when the last reader leaves, as any unsaved words do.

**Acceptances do not outlive their use.** The save removes the acceptances it stored, in the same act. A document dropped with nothing left to store removes the acceptances it holds. Every save made as the prose role, and every start of the application, removes the acceptances past their hour, through a store act that removes only those and answers nothing.
