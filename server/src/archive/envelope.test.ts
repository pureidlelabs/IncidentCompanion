...
/**
 * A minimum to seal against. **A literal, not `policy/keys.ts`:** `archive/`
 * reaches nothing, and a test that imports what the module may not is the same
 * violation one line further out.
 */
const PASSPHRASE_CHARS = 12
...
async function withWorkFactor(logN: number): Promise<Buffer> {
  const sealed = await seal(PLAIN, PASS, PASSPHRASE_CHARS)
...
describe('sealing', () => {
  it('refuses a passphrase too short to be one', async () => {
    ...
    await expect(seal(PLAIN, 'short', PASSPHRASE_CHARS)).rejects.toBeInstanceOf(WeakPassphrase)
    await expect(seal(PLAIN, '', PASSPHRASE_CHARS)).rejects.toBeInstanceOf(WeakPassphrase)
    await expect(seal(PLAIN, 'x'.repeat(PASSPHRASE_CHARS), PASSPHRASE_CHARS)).resolves.toBeInstanceOf(Buffer)
  })
  ...
  it('produces a different archive every time from the same input', async () => {
    ...
    const a = await seal(PLAIN, PASS, PASSPHRASE_CHARS)
    const b = await seal(PLAIN, PASS, PASSPHRASE_CHARS)
  ...
  it('is recognisable as an archive rather than a plain zip', async () => {
    expect(isSealed(await seal(PLAIN, PASS, PASSPHRASE_CHARS))).toBe(true)
...
describe('opening', () => {
  it('returns exactly what was sealed', async () => {
    expect((await open(await seal(PLAIN, PASS, PASSPHRASE_CHARS), PASS)).equals(PLAIN)).toBe(true)
  })

  it('carries an empty archive', async () => {
    expect((await open(await seal(Buffer.alloc(0), PASS, PASSPHRASE_CHARS), PASS)).length).toBe(0)
  })
  ...
  it('carries an archive larger than one stream chunk', async () => {
    ...
    expect((await open(await seal(big, PASS, PASSPHRASE_CHARS), PASS)).equals(big)).toBe(true)
  })
  ...
  it('refuses an archive truncated to a shorter one', async () => {
    ...
    const sealed = await seal(big, PASS, PASSPHRASE_CHARS)
