...
export interface Session {
  ...
  /**
   * Whether the account holds the role every `@AdminOnly` route demands. A
   * claim about what the server will accept, never a check: absent means no.
   */
  readonly admin?: true
  /**
...
function restore(): Session | null {
  try {
    ...
    const { userId, username, demo, admin } = parsed as Record<string, unknown>
    ...
    // Carried, or every reload draws an administrator the analyst's rail
    // until the boot probe lands.
    const held = { userId, username, ...(admin === true ? { admin: true as const } : {}) }
    return demo === true ? { ...held, demo: true } : held
  } catch {
