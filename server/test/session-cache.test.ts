...
describe.skipIf(!RUNNABLE)('a session held in Redis', () => {
  ...
  const whoAmI = async (cookie: string): Promise<number> => {
  ...
  /**
   * A session written just now. A get-session read does not write one back --
   * which is the third test's claim -- so a test needing a key present makes
   * its own rather than inheriting whatever ran before it.
   */
  const freshSession = async (): Promise<string> => (await signIn(harness, admin.email)).cookie
  ...
  it('writes the session into Redis under its own prefix', async () => {
    ...
    expect(await whoAmI(await freshSession())).toBe(200)
  ...
  it('still authenticates after Redis loses every session key', async () => {
    ...
    expect(await whoAmI(cookie)).toBe(200)
