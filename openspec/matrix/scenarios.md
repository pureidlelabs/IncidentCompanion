...
| A case is one case however it is named | An analyst names the case in another spelling | demonstrated | server/test/a-case-is-one-room-however-it-is-spelled.test.ts :: a case named in capitals over the live connection > puts the analyst in the roster the others see ; server/test/a-case-is-one-room-however-it-is-spelled.test.ts :: a case named in capitals over the live connection > refuses the case deletion while they are connected, by either spelling of the route |
...
| A caret names the analyst the install admitted | A connection sends for a caret another analyst holds | demonstrated | server/test/a-caret-carries-the-name-the-install-knows.test.ts :: a caret drawn from the live connection > leaves a caret another connection announced where it was, and relays nothing sent for it |
| A connection is held to a rate | One analyst opens many connections at once | demonstrated | server/test/a-connection-has-a-bounded-rate.test.ts :: a connection to the case, at a rate > admits a bounded number of one analyst's upgrades made at once, and records each refusal once |
| A connection is held to a rate | Many connections nobody signed | demonstrated | server/test/a-connection-has-a-bounded-rate.test.ts :: a connection to the case, at a rate > refuses anonymous upgrades past a bound per address, and writes a bounded number of lines |
