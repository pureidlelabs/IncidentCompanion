...
describe('the Article 33 deadline', () => {
  ...
  it('is unknown rather than thrown for an unparseable stamp', () => {
    // `gdprAwareAt` reaches the client from a CSV import and the API as well
    // as from a form.
    expect(deadline('not a date')).toBeNull()
  })

  it('reads an offsetless stamp as UTC, not as the viewer local time', () => {
    // `new Date('...T00:00:00')` reads local, which moves the deadline by the
    // viewer's offset. The stamp is stored without one and means UTC.
