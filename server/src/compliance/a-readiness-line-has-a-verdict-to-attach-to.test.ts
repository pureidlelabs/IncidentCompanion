 ...
 * verdict rows are*. It was two lists of conditions rather than one, and they
 * had parted on DORA -- the verdict waits for the Article 6 gate to have any
 * answer and the readiness line did not.
 *
 * **Nothing reached an analyst either way**, because the only consumer of a
 * readiness line attaches it to a verdict row it is already emitting and drops
 * the rest. This holds the two to one set so they cannot part where it would
 * show. -> #647
 *
