...
export class LibraryController {
  ...
  /**
   * **An unknown slug is a 404 naming what is served.**
   */
  ...
  async apply(
    ...
    /**
     * **A library with no payload schema refuses a document, exactly as it refuses
     * a New.**
     */
  ...
  async listing(@Param('slug') slug: string) {
    ...
    return {
      ...
      /**
       * **Present and empty, always.**
       */
      problems: [],
      /**
       * What a New offers to start from.
       */
      startOptions: [
  ...
  async create(@Param('slug') slug: string, @Body() body: unknown): Promise<Written> {
    ...
    /**
     * **Blank is the schema's own defaults, not an empty object literal.**
     */
    let source: unknown = {}
    if (startFrom) {
  ...
  async remove(@Param('slug') slug: string, @Param('name') name: string): Promise<Written> {
    ...
    /**
     * **A built-in is refused rather than silently kept.**
     */
    if (existing.builtin) {
      throw new BadRequestException({
