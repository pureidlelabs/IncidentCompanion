...
**One policy per command.** Every table holding a case's rows answers a row only to a scope naming its case and a principal who reaches it: read to see a row, write to add, change or remove one. A case itself is answered by its own customer, and destroying one needs delete. A visit to a case is its analyst's alone while they reach the case, and removing one asks only whose it is, so a list pruned after reach was withdrawn still drops what it no longer shows. A record the first reader raises from defaults may be raised at read. The record of who changed a case is read and added to, never changed or removed by the application, and goes with its case when the case is deleted. Which customer a case answers for is changed only by the store's own acts, which hold the move's rules, and by the seeding identity: the serving identity holds no privilege to change it with an ordinary write, and the store refuses the change to any other writer whatever privileges it holds. Of a case's own row, the serving identity changes only the fields a case edit sets and what that write stamps. Every privilege the serving, seeding and prose identities hold on a table is set by the schema application, table by table, and a table it names nothing for stops the application. It first withdraws every table privilege those identities and everybody hold, so a grant made on a table out of band does not survive it. Provisioning the identities grants none, so doing it again at any moment changes none. Making one identity a member of another out of band is an administrator's act, and the schema application does not undo it. A write the store refuses for reach, inside the scope that names a case, is answered as that case not being there, with the refusal kept as its cause.
