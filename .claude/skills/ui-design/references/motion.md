...
- **The skeleton shimmers, and that is the built behaviour** -- `skeleton.tsx` travels a highlight across, on a 1.4s linear loop. This entry used to say a skeleton pulses and does not shimmer; the tree disagreed with it and the tree is what ships. The argument it was protecting still holds and is worth keeping in mind: a travelling highlight over a full table is thirty elements animating at once, so the question for a *new* loading surface is how many of them there will be, not whether shimmer is allowed.
