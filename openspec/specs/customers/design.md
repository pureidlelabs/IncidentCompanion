...
**The reference check is a boundary at every door that writes one, and the merge is one of them.** A reference is unique within its customer and nowhere wider: the reference is the customer's own ITSM ticket, so two organisations legitimately share a ticket number and the rule has to be asked per customer rather than per install. A merge refuses to bring two cases carrying one reference under a single customer, and the doors that raise or edit a case refuse the same state for the same reason.

**Which customer a case is in has to be a value the row holds**, or the rule has no group to be asked about. A case that carried nothing and resolved to the default when it was read left every unattributed case in one group by inference and none by storage, which is a rule the database cannot enforce and two cases can slip through.
