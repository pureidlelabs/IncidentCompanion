 ...
 *
 * **The error is not unwrapped here.** `add_entries` reports `"row N: ..."`
 * for a bad row and something row-less for a cap breach; mapping the former
 * back to a preview row is `csv-import.ts`'s `parseRowError` /
 * `previewIndexForServerRow`, kept out of the network layer so it stays a
 * pure function a test can hold without a fetch mock.
 */
