...
The lock lives where a password is verified rather than at any door, so every door that checks one counts into the same runs and refuses a locked run's right password as a wrong one, including a door added later. Every refusal there runs the same statements whether its run is open or locked, so a locked run costs what a wrong password does, and the record of a failure at an address with no account spends them too. Failures arriving together are each counted, and the lock is taken only from the count its failure saw, so they lock the run once. A run whose count has reached the threshold is shut from that count on, before its lock is written, so a right password arriving in that gap is refused. A run is keyed by the caller's whole address, never by the network it sits on: one IPv6 network is one LAN, not one machine.
