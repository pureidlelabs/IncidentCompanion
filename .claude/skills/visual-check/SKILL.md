...
**That second page's text is not a diagnosis, whatever it says.** Its handler prints the same advice about `allowedHosts` and the hostname for every failure to load the preview script, under two conditions and no others: the host is not the literal `localhost`, and the config is a development one. The default `127.0.0.1` meets the first — but **`STORYBOOK_URL=http://localhost:6007` is a documented invocation, and under it the handler draws nothing at all**, so the detector goes blind and the tier is back to reporting a geometry failure against whichever story was running. Against a built Storybook the branch is dead for the second reason. **Use the `127.0.0.1` default unless you have a reason not to**, and read a clean sweep at `localhost` as unproven rather than clean. So `brokenPreview` asks the server again and reports what it answers: a status when it refused, a throw when the connection did, and `re-fetched 200` when the entry script itself is fine. **That last one is not proof the failure was transient** — the `error` event fires for a failure anywhere in the module graph, so a story file or decorator that will not resolve leaves the entry script perfectly serveable.
...
**An empty baseline reads as reassurance, which is why the run says so in different words.** With nothing to compare against it prints *"no baseline recorded"* rather than a count, and an undecodable capture prints `!` rather than counting as unchanged. Both shapes cost real coverage: a baseline of 80 views silently becoming 16 leaves the next sweep reporting no differences while holding none of them. If you changed a page and the run reports no difference, check `server/.visual/baseline` actually holds that view before believing it — the same empty-set shape that makes a test pass over nothing.
...
- **A `DataTable` is a window, not a table.** `DataTable` windows its own rows from `VIRTUALIZE_FROM` up, hand-rolled with spacers rather than by a library (`ui/src/components/blocks/data-table.tsx:214`), and only at `scroll="box"` — so its DOM row count is a window, never a total. **The Timeline is not one of them**: it draws a plain `<ol>` over every run, so its DOM holds the whole list. Ask which of the two you are looking at before reading a row count as evidence of either.
- **The tier drives the built bundle, so `npm run build` in `ui` comes before every sweep.** The browser tier's `baseURL` is the Node server, which serves `ui/dist` as static assets - `unservedReason` refuses a Vite dev server outright, because a shell whose script is `/src/main.tsx` is the one thing this tier must not be pointed at. Vite on its own port is a different server that nothing here talks to. **A stale capture is pixel-identical to a correct capture of code that has not changed**, so a palette fix can be made, captured twice and read back as not having applied while Vite serves the new class string all along. `asPersona` refuses the run rather than reporting on the previous build, so this is a one-line remedy rather than a trap - but the build is still yours to run.
...
- **An element that takes no pointer is not a target, unless it is merely disabled.** `overlap`, `offscreen` and `small-target` skip anything whose computed `pointer-events` is `none` — a marker like `OverlayAnchor`, painted over the thing it points at so the collision is the design. **Disabled controls are the exception and it is load-bearing**: this kit draws `isDisabled` with `pointer-events-none`, so excluding on the property alone would drop every dimmed button, toggle, select, disclosure, stepper, input and tab from all three checks — a disabled control is painted at full size and can be undersized or cut off exactly as an enabled one can. The predicate is `[disabled], .disabled, [aria-disabled="true"]`, the same one the contrast check uses, and it needs both spellings: a button takes the native attribute, a tab only `aria-disabled`. **The control set is `button, a[href], input, [role="button"], [role="tab"]` and nothing else**, so a menu item, option, row or tree item was never in these three checks to begin with — and a *disabled* link is not an `<a>` at all, since React Aria renders it as a `span`. What no check can see is a control made unclickable by mistake while claiming to be enabled.
...
- **Page zoom.** A browser at 125% is a different layout, and nothing here changes it. Density is a different axis and is covered for the sweeps: `playwright.visual.config.ts` and the affordance audit take `densityProjects()` from `server/e2e/visual/densities.ts` and run at 100%, 125%, 150% and 200%, held there by `tests/repo/test_visual_runs_at_retina.py`. **The Storybook walk is the exception**, and runs at one density: `probe.js` reads through `getBoundingClientRect` and `getComputedStyle`, so every ratio answers alike -- measured over 452 renders at 1 and 2, identical findings and identical duplicate clusters. **A capture at one density is not evidence about another** -- a layout on a fractional CSS pixel rounds differently at each, so a row bleeding through a sticky header at 200% is invisible at 100%. `VISUAL_DENSITIES=1,2` trims the set where a sweep's runtime matters more than its coverage.
- **Telling a duplicate frame from a duplicate story.** The oracle pairs stories whose captures hash alike, and a pair is a question rather than a finding: it says these two frames are the same, not that one story is redundant. Three causes account for most of them, each measured. A `play` that tears down what it opened leaves every sibling on the same frame -- `Blocks/Dialogs/About`'s `Open`, `Reading` and `Refused` all end `await closed(canvasElement)`, so the oracle sees one closed dialog three times. A state whose only visible difference is a toast hashes identical, because `stripToasts` in `view.ts` removes the region outright and runs before the capture. And a state that is invisible at rest pairs with the state it differs from: `Components/Calendar`'s `ReadOnly` says so in its own docstring and proves itself through `play` instead. **Read the pair's stories before reading it as waste.**
- **A narrow viewport, except where a story asks for one.** The walk resizes only for a story carrying a `viewport` global, and the tree holds five, each named *A narrow viewport*: `sign-in`, `first-run`, `overview`, `change-password` and `case-record-form`. **One of the five renders inside the app shell**, so whatever the shell does at a narrow viewport is exercised by a single story. Everything else is walked at 1440 and 720 from a fresh load. **And `applyStoryViewport` resizes after the story has loaded**, so a finding on one of those five is about a viewport that *changed*, not one the screen opened at -- the two differ, and #902 is the case where they differ by the whole layout.
