...
- **`hover:` is already React Aria's hover.** `tailwindcss-react-aria-components` with no prefix redefines `hover:`, `focus:`, `focus-visible:`, `disabled:` and `pressed:` as `:is(:where([data-rac])[data-hovered], :where(:not([data-rac])):hover)` — the `data-*` state on an Aria element, the pseudo-class on anything else — so a `hovered:` spelling compiles to the same CSS and a disabled Aria control does not light under the pointer. The one variant it does not unify is `active:`, which on an Aria element never fires (`preventDefault` on pointerdown); write `pressed:`. `server/e2e/visual/states.storybook.spec.ts` drives the kit's stories and reads the paint back, which is the only test that can see any of this.
- **The browser tier raises its own stack, and it is fail-closed only when asked.** `server/e2e/playwright.config.ts` declares a `webServer` running `dev-node.sh --keep-data`, reusing a session somebody already has -- so `./test.sh --browser` and `./verify.sh --detailed` need nothing started first. `BASE` is the **Vite** port, not the API one: `ui/dist` is built for `VISUAL_TARGET=dist` and read by nothing else, and both launchers used to probe the API port while Playwright drove Vite's. Its per-spec skips are silent by design -- a run against no Storybook omits most of the tier and still exits 0 -- so `IC_SUITE_MUST_RUN` arms `server/e2e/support/prerequisites.ts`, which refuses the run instead. -> #89
