 ...
 * answered 200, so the client believes it saved. That is exactly how the figure
 * block behaved before this: the picker collected an evidence id, kept it in
 * component state, and there was nowhere for it to go.
 *
...
describe('a report block', () => {
   ...
   * `z.uuid()` here is invisible to it - which is what this field was. Through
   * the generic collection route that meant a body naming *another case's*
   * evidence was accepted and stored, because the database's own foreign key
   * is checked outside row-level security; and an id naming no row raised a raw
   * Postgres violation where every other reference answers a clean refusal.
   *
   ...
   * pick none - and the client committed `''` for it, which this schema refuses
   * and the route answers 400 to. On an ordinary click, in the shipping UI.
   * `null` is what the column, the `nullish` here and the foreign key's own
   * `set null` all mean by "no image".
   */
