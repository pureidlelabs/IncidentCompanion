 ...
 *
 * **Derived, never written down**, for the reason `e2e/playwright.config.ts`
 * gives about every other port: `stack.mjs` allocates a slot from the
 ...
 *
 * `__dirname` rather than `import.meta`, as the config next to it: Playwright
 * loads these through a CommonJS wrapper whatever the extension says.
 */
