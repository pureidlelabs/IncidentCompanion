 ...
 * there was one here for months and the console said so on every load while
 * it silently did nothing. `/api/docs/boot.js` is the same answer to the same
 * problem, one directory over.
 *
 * **`next-themes` cannot do this job in a client-rendered app, and that is
 * structural rather than a gap.** Its `ThemeScript` is a `<script>` element
 * rendered by React, so it is hoisted and run when the bundle executes - long
 * after first paint - and its `nonce` is applied only when
 * `typeof window === 'undefined'`, which never holds here. The library still
 * owns the state, the `system` listener and the persistence; this reads the
 * same key it writes.
 *
