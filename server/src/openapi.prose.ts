/**
 * The English in the OpenAPI document: what a tag, a resource, an operation
 * and a refusal are called, derived from the path and the method so a route
 * added tomorrow reads like the ones beside it.
 *
 * Pure functions over strings and schemas; `openapi.ts` builds the document
 * and asks these for its prose.
 */
import { z } from 'zod'
 ...
 */
export const PARAMETERS: Readonly<Record<string, string>> = {
...
export function asEmpty(operation: Operation, method: string, path: string): boolean {
...
export function asEnvelope(operation: Operation, method: string, path: string): boolean {
...
export function asUpload(operation: Operation, method: string, path: string): boolean {
...
export function asDownload(operation: Operation, method: string, path: string): boolean {
