...
Where a row came from, as the doors that write rows answer it. Not what found the row — the tool, the rule, the analyst who saw it first — which no column holds and which a travelled case is where the difference shows. → #727
...
**A stamp is narrowed to the columns the target table has, and that narrowing is the whole mechanism.** A field naming no column is dropped by the query builder without a word, so a stamp written unconditionally is silent on the tables that lack the column and puts a field those tables do not have into the record of what was written. Nothing else fails when the narrowing is wrong, so it is what a test holds, quantified over the tables an import writes.

**One answer for the four doors, and it sits below all of them.** The doors are in four folders and no two of them may reach each other; what they share is the schema whose columns the stamp answers about. A helper beside any one door would need an edge that the layering exists to refuse.
