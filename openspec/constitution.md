...
| --- | --- | --- | --- |
| Article III, cross-case reach | Any authenticated analyst can reach any case. | Not built. The control was deferred on difficulty, never decided against. | An access rule narrower than "the case exists", enforced in the single place Article III requires. |
...
| ASVS 5.0 L2, multi-factor authentication | A second factor is available but not required by default. | Article IV. Whether to require it is the operator's decision, taken against their own users and risk. The install says plainly that leaving it off falls short of L2. | The operator turning the policy on. Nothing in the product changes. |
