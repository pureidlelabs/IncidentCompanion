...
| Administrative events are logged | A change to what already stands | demonstrated | server/test/an-act-that-changed-nothing-is-not-logged-as-done.test.ts :: an administrative act that changed nothing > logs a hold and a grant once when asked for twice, and a revocation and a release that happened ; server/test/an-act-that-changed-nothing-is-not-logged-as-done.test.ts :: an administrative act that changed nothing > logs no account state or role the account already had |
