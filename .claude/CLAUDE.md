...
- **`hover:` is already React Aria's hover.** The Aria Tailwind plugin, unprefixed, compiles `hover:`, `focus:`, `focus-visible:`, `disabled:` and `pressed:` to the `data-*` state on an Aria element and the pseudo-class on anything else, so `hovered:` is the same CSS. `active:` is the exception: it never fires on an Aria element, so write `pressed:`. `states.storybook.spec.ts` is the test that can see this.
- **The browser tier raises its own stack, and it is fail-closed only when asked.** `server/e2e/playwright.config.ts` declares a `webServer` running `dev-node.sh`, reusing a session somebody already has -- so `./test.sh --browser` and `./verify.sh --detailed` need nothing started first. `BASE` is the **Vite** port, not the API one: `ui/dist` is built for `VISUAL_TARGET=dist` and read by nothing else, and both launchers used to probe the API port while Playwright drove Vite's. Its per-spec skips are silent by design -- a run against no Storybook omits most of the tier and still exits 0 -- so `IC_SUITE_MUST_RUN` arms `server/e2e/support/prerequisites.ts`, which refuses the run instead. -> #89
