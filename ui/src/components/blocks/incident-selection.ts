 ...
 *
 * **An event always names its severity; anything else only names one it has.**
 * The timeline draws `unset` for an unrated event, so a summary that omitted
 * the segment described one entry two ways -- and omitted it on exactly the
 * event still waiting to be rated. -> #983
 */
export function selectionSummary(node: IncidentNode): string {
  ...
  if (node.kind === 'event') parts.push(severityLabel(node.severity))
